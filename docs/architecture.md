...
В браузере проверка сроков работает в открытом окне каждые 10 секунд. В Android APK `AlarmManager` хранит разрешённое расписание отдельно от WebView; при наличии точного доступа используется `setAlarmClock`, который будит устройство и не сдвигается пакетной экономией батареи. Receiver запускает foreground alarm service: он выводит ongoing heads-up/полноэкранное системное уведомление с Done/Snooze, проигрывает зацикленный системный alarm stream и повторяет вибрацию даже при закрытом WebView. Epoch-время передаётся в мост как 64-битное целое, поэтому не теряется при JSON-переходе между WebView и Java. Действия сохраняются в native storage до записи в IndexedDB и последующей синхронизации, поэтому краткое отсутствие сети не теряет изменение. Экран клиента показывает статусы уведомлений, точных будильников и fullscreen-доступа, умеет немедленно проверить сигнал и поставить безопасный тест на 15 секунд. Без точного доступа используется разрешённый Android fallback с возможной задержкой.
