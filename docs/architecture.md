...
Проверка сроков работает в открытом окне каждые 10 секунд. Системные уведомления требуют разрешения пользователя; напоминание остаётся в списке, даже если ОС отклонила показ. Статус FIRED синхронизируется, Done/Snooze выполняются локально и отправляются позже. В фоне/при выключенном устройстве точного планировщика пока нет.
