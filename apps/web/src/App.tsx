...
import { ArrowDownToLine, ArrowRight, Archive, Bell, Check, CheckCheck, ChevronRight, CircleHelp, Cloud, CornerDownLeft, FileText, Inbox, Laptop, Menu, Plus, RefreshCw, Search, Settings2, ShieldCheck, Sparkles, Terminal, Wifi, WifiOff, X } from 'lucide-react';
...
import { MAX_TEXT, statusLabels, type Note, type Reminder } from '../../../packages/shared/src/index';
...
export default function App() {
  ...
  const [query,setQuery]=useState(''); const [inboxFilter,setInboxFilter]=useState('all');
  ...
  async function capture(event?:FormEvent) {
    ...
    try {await saveNote(text);setText('');setMessage('Сохранено локально. Мысль в безопасности.');void scheduleSync();captureRef.current?.focus();}
    ...
    <div className="workspace">
      ...
      <main>
        ...
        {page==='capture'&&<>
          ...
          <div className="capture-grid"><section className="capture-panel"><form onSubmit={capture}><div className="panel-top"><span><span className="status-dot"/> НОВАЯ ЗАПИСЬ</span><span>RAW / TEXT</span></div><label className="sr-only" htmlFor="thought">Что сейчас в голове?</label><textarea ref={captureRef} id="thought" value={text} disabled={busy} onChange={e=>setText(e.target.value)} onKeyDown={e=>{if((e.ctrlKey||e.metaKey)&&e.key==='Enter'){e.preventDefault();void capture();}}} placeholder={'Что сейчас в голове?\n\nИдея, задача, случайная мысль…'} maxLength={MAX_TEXT}/><div className="editor-meta"><span>{text?'Черновик · ещё не сохранён':'Не нужно выбирать папку или теги'}</span><span>{text.length.toLocaleString('ru')} / 100 000</span></div><div className="capture-actions"><span className="shortcut"><kbd>Ctrl</kbd> + <kbd>↵</kbd><span>сохранить</span></span><button className="primary" type="submit" disabled={busy||!text.trim()}>{busy?'Сохраняем…':'Сохранить мысль'}<CornerDownLeft size={17}/></button></div></form><div className="capture-footnote"><ShieldCheck size={14}/><span>Сохраняется на устройстве. Работает без интернета.</span></div></section>
        ...
        {page==='ask'&&<><PageHeading code="SEARCH / 03" title="Вернитесь к мысли" description="Найдите то, что уже записали. Поиск работает офлайн."/><div className="search-box"><Search size={21}/><input aria-label="Поиск по заметкам" value={query} onChange={e=>setQuery(e.target.value)} placeholder="Слово или фраза из вашей заметки…" autoFocus/>{query&&<button className="icon-button" aria-label="Очистить поиск" onClick={()=>setQuery('')}><X size={18}/></button>}</div><div className="search-caption">{query?`Найдено записей: ${foundNotes.length}`:'Поиск по оригинальному тексту всех неархивных заметок'}</div>{query?<div className="note-list">{foundNotes.map(n=><NoteRow key={n.id} note={n} onOpen={()=>setSelected(n)}/>)}</div>:<Empty icon={<Search size={30}/>} title="Всё начинается с одного слова" text="Идея проекта, название книги, случайная фраза. Введите то, что помните."/>}{query&&!foundNotes.length&&<Empty icon={<Search size={28}/>} title="Совпадений нет" text="Попробуйте более короткое слово или проверьте архив во входящих."/>}<div className="future-note"><Sparkles size={18}/><div><strong>Ask Vault — следующий этап</strong><p>Ответы AI по вашему Obsidian появятся после подключения локального AI-моста. Сейчас доступен поиск по тексту.</p></div></div></>}
      ...
      <footer className="workspace-footer"><span><span className="status-dot"/> LOCAL FIRST. ALWAYS.</span><span>VAULT TERMINAL <b>v0.1.0</b></span><button className="text-button" onClick={()=>setSyncPanel(true)}>{queue.length?`${queue.length} в очереди`:'Очередь пуста'}<ArrowRight size={12}/></button></footer>
    </div>
...
function NoteRow({note,onOpen}:{note:Note;onOpen:()=>void}) {return <button className="note-row" onClick={onOpen}><span className="note-icon"><FileText size={19}/></span><span className="note-preview"><strong>{note.text.split('\n').find(t=>t.trim())}</strong><span>{date(note.clientCreatedAt)}<span className="note-id"> / {shortId(note.id)}</span></span></span><span className={'note-status '+(note.status==='SYNC_ERROR'?'warning':'')}>{note.serverReceivedAt?<CheckCheck size={14}/>:<span className="small-dot"/>}{statusLabels[note.status]}</span><ChevronRight className="note-arrow" size={17}/></button>;}
...
function SettingsPanel({settings,report,notify,onExport}:{settings:Settings;report:(p:Promise<unknown>)=>void;notify:(s:string)=>void;onExport:()=>void}) {
  ...
  return <div className="settings-grid"><section className="settings-card"><h2><Terminal size={20}/>Внешний вид</h2><p>Терминал с характером или спокойная классика.</p><div className="theme-picker">{(['terminal','classic-dark','classic-light'] as const).map((theme,i)=><button key={theme} aria-pressed={settings.theme===theme} className={'theme-option '+(settings.theme===theme?'selected':'')} onClick={()=>report(update({theme}))}><span className={'theme-swatch swatch-'+theme}><span/><i/><i/></span>{['Терминал','Тёмная','Светлая'][i]}</button>)}</div><label className="setting-row">Линии CRT<input type="checkbox" checked={settings.scanlines} onChange={e=>report(update({scanlines:e.target.checked}))}/></label><label className="setting-row">Свечение<input aria-label="Свечение" type="range" min="0" max="100" value={settings.glow} onChange={e=>report(update({glow:Number(e.target.value)}))}/></label><label className="setting-row">Уменьшить движение<input type="checkbox" checked={settings.reducedMotion} onChange={e=>report(update({reducedMotion:e.target.checked}))}/></label><label className="setting-row">Размер текста<select value={settings.fontSize} onChange={e=>report(update({fontSize:e.target.value as Settings['fontSize']}))}><option value="s">Маленький</option><option value="m">Средний</option><option value="l">Большой</option></select></label></section>
    <section className="settings-card"><h2><Cloud size={20}/>Устройство и сервер</h2><p>Пустой адрес использует сервер приложения.</p><form className="form-stack" onSubmit={e=>{e.preventDefault();report((async()=>{const clean=url.trim().replace(/\/+$/,'');if(clean){const parsed=new URL(clean);if(parsed.username||parsed.password||parsed.search||parsed.hash||parsed.pathname!=='/')throw new Error('Укажите только адрес сервера, без пути и паролей');if(parsed.protocol!=='https:'&&!(['localhost','127.0.0.1','[::1]'].includes(parsed.hostname)&&parsed.protocol==='http:'))throw new Error('Для удалённого сервера требуется HTTPS');}await update({deviceName:name.trim()||'Моё устройство',serverUrl:clean,vaultName:vault.trim()});notify('Настройки подключения сохранены');await scheduleSync();})());}}><label>Имя устройства<input maxLength={100} value={name} onChange={e=>setName(e.target.value)}/></label><label>Адрес сервера<input type="url" value={url} onChange={e=>setUrl(e.target.value)} placeholder="Текущий сервер"/></label><label>Название Obsidian vault<input value={vault} onChange={e=>setVault(e.target.value)} placeholder="Для ссылки «Открыть в Obsidian»"/></label><button type="submit" className="secondary">Сохранить подключение</button></form><div className="pairing"><label>Токен подключения<input type="password" autoComplete="off" value={token} onChange={e=>setToken(e.target.value)} placeholder="Из команды device:add"/></label><button className="secondary" disabled={!token} onClick={()=>report(api('/api/v1/session',{method:'POST',body:JSON.stringify({token})}).then(()=>{setToken('');notify('Устройство подключено');return scheduleSync();}))}>Подключить устройство</button><small>Токен не сохраняется в браузере. Подключение использует защищённую cookie-сессию.</small></div></section>
