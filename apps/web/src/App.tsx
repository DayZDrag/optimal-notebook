...
export default function App() {
  ...
  const [nativeReminderAccess,setNativeReminderAccess]=useState<NativeReminderStatus>();
  ...
  useEffect(()=>{
    const start=()=>{setOnline(navigator.onLine);void scheduleSync().catch(()=>{});void applyNativeReminderActions();if(isNativeAndroid())void nativeReminderStatus().then(setNativeReminderAccess).catch(()=>{});};
  ...
  useEffect(()=>{if(isNativeAndroid())void nativeReminderStatus().then(setNativeReminderAccess).catch(()=>{});},[]);
  useEffect(()=>{report(reconcileNativeReminders(reminders));},[reminders]);
  useEffect(()=>{
    if(!settings)return;
  ...
  useEffect(()=>{
    if(!settings)return;
    // AlarmManager owns Android delivery. Do not let the WebView mark an item
    // FIRED first, otherwise opening the app near its deadline could cancel
    // the system alarm before Android has shown it.
    ...
    const fire=async()=>{
      for(const item of due.filter(r=>r.status!=='FIRED')) {
        ...
        if(!isNativeAndroid() && settings.notifications && 'Notification' in window && Notification.permission==='granted') {
          try {
  ...
  async function applyNativeReminderActions() {
    const actions=await readNativeReminderActions();const acknowledged:string[]=[];
    for(const action of actions) {
      const current=await db.reminders.get(action.id);if(!current)continue;
      const next=action.status==='SNOOZED'&&action.remindAt ? {...current,status:'SNOOZED' as const,remindAt:new Date(action.remindAt).toISOString()} : {...current,status:action.status};
      if(next.status===current.status && next.remindAt===current.remindAt) {acknowledged.push(action.id);continue;}
      await saveReminder(next);acknowledged.push(action.id);
    }
    await acknowledgeNativeReminderActions(acknowledged);
    if(acknowledged.length)void scheduleSync();
  }
  async function enableSystemReminders() {
...
function SettingsPanel({settings,report,notify,onExport,nativeReminderAccess,onEnableNativeReminders}:{settings:Settings;report:(p:Promise<unknown>)=>void;notify:(s:string)=>void;onExport:()=>void;nativeReminderAccess:NativeReminderStatus|undefined;onEnableNativeReminders:()=>void}) {
  ...
  return <div className="settings-grid"><section className="settings-card"><h2><Terminal size={20}/>Внешний вид</h2><p>Терминал с характером или спокойная классика.</p><div className="theme-picker">{(['terminal','classic-dark','classic-light'] as const).map((theme,i)=><button key={theme} aria-pressed={settings.theme===theme} className={'theme-option '+(settings.theme===theme?'selected':'')} onClick={()=>report(update({theme}))}><span className={'theme-swatch swatch-'+theme}><span/><i/><i/></span>{['Терминал','Тёмная','Светлая'][i]}</button>)}</div><label className="setting-row">Линии CRT<input type="checkbox" checked={settings.scanlines} onChange={e=>report(update({scanlines:e.target.checked}))}/></label><label className="setting-row">Свечение<input aria-label="Свечение" type="range" min="0" max="100" value={settings.glow} onChange={e=>report(update({glow:Number(e.target.value)}))}/></label><label className="setting-row">Уменьшить движение<input type="checkbox" checked={settings.reducedMotion} onChange={e=>report(update({reducedMotion:e.target.checked}))}/></label><label className="setting-row">Размер текста<select value={settings.fontSize} onChange={e=>report(update({fontSize:e.target.value as Settings['fontSize']}))}><option value="s">Маленький</option><option value="m">Средний</option><option value="l">Большой</option></select></label></section>
    ...
    <section className="settings-card"><h2><Bell size={20}/>Уведомления</h2>{isNativeAndroid()?<><p>Системный будильник, вибрация и всплывающее уведомление Android работают даже после закрытия приложения.</p><NativeReminderInfo access={nativeReminderAccess} onEnable={onEnableNativeReminders}/></>:<><p>Напоминания в открытом приложении. Доставка при закрытом приложении появится с Push / Android.</p><button className="secondary" onClick={()=>report((async()=>{if(!('Notification' in window))throw new Error('Этот браузер не поддерживает уведомления');const permission=await Notification.requestPermission();await update({notifications:permission==='granted'});notify(permission==='granted'?'Уведомления включены':'Уведомления запрещены браузером; напоминания доступны в приложении');})())}>{settings.notifications?'Проверить разрешение':'Разрешить уведомления'}</button>{settings.notifications&&<button className="text-button" onClick={()=>report(update({notifications:false}))}>Выключить уведомления</button>}</>}</section>
