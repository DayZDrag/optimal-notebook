...
- MVP-6: создана Capacitor Android-обёртка и проверенная debug APK 0.1.5 для API 24+. Она содержит offline UI, очередь, экспорт в Documents/Share, deep-link scheme, нативную вставку токена, системный голосовой ввод и проверяемое подключение к Vercel. Название заметки хранится как Markdown-заголовок RAW. SAF, local notifications и Share Sheet как входной канал ещё не реализованы.
- MVP-7: реализовано безопасное одностороннее зеркало разрешённого Markdown с SHA-256, сохранением серверных версий и поиском. Автоматическая двусторонняя синхронизация, вложения и auto-merge конфликтов намеренно не реализованы.
