...
- MVP-1: реализованы React/PWA, IndexedDB, постоянная очередь, Hono, серверный журнал, retry и экспорт RAW. Локальная разработка использует SQLite; Vercel production — Neon PostgreSQL без DDL в request path и с передачей POST-body напрямую в Hono.
...
- MVP-6: создана Capacitor Android-обёртка и проверенная debug APK для API 24+. Она содержит offline UI, очередь, экспорт в Documents/Share, deep-link scheme, нативную вставку токена и проверяемое подключение к Vercel. SAF, local notifications и Share Sheet как входной канал ещё не реализованы.
